...
void debug_draw_t::DrawSolidPolygon(const b2Vec2* vertices, int32 vertexCount, const b2Color& color)
{
	...
	{
		glVertex2f(vertices[i].x, vertices[i].y);
		//Setting the color of area near each vertex of a polygon to green
		// The pool table is green because of this fragment 
		glColor4f(0,1,0,0.4);
	}
	glEnd();
	glDisable(GL_BLEND);
...
void debug_draw_t::DrawSolidCircle(const b2Vec2& center, float32 radius, const b2Vec2& axis, const b2Color& color)
{
	...
	/// # Coloring the balls
	//!Used glColor3f function to color the spherical objects
	
	// This code fragment colors the balls   
	if(radius == 1.0000f){
		// Setting the color of blue balls
		glColor3f(0,0,1);;
	}
	else if(radius == 1.0001f){
		// Setting the color of other balls
		glColor3f(0.9,0.5,0.3);
	}
	else if(radius ==  1.5f || radius == 1.0002f)
		//Setting the color of pockets and cue ball
		glColor3f(0,0,0);
	else if(1.0f < radius && radius < 1.0001f)
		//Setting the color of white ball
		glColor4f(1,1,1,1.0f);
	...
	// Setting the boundary color for each object to be White
	glColor4f(1,1,1,1.0f);
